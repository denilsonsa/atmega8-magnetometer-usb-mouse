\documentclass[brazil,pagestart=firstchapter]{abntex2}
...
\centerline{\textbf{\large \imprimirtitulo}}

\bigskip
\bigskip

\centerline{\textbf{\imprimirautor}}
...
\item Clock máximo de \qty{16}{\mega\hertz} usando um cristal externo.
...
\qtyproduct[product-units=single]{3.0 x 3.0 x 0.9}{\milli\metre} de tamanho.
É impossível trabalhar manualmente com algo tão minúsculo, por isso foi
...
(\qty{1.5}{\mega\bit\per\second}) e \textit{full speed}
(\qty{12}{\mega\bit\per\second}). Alguns anos depois, em setembro de 1998, foi
...
\textit{high speed} (\qty{480}{\mega\bit\per\second}) -- mas manteve total
...
\qty{+5}{\volt} e \GND) e duas para comunicação (D+ e D-). Os dados são
...
uma quarta velocidade: \textit{super speed} (\qty{5}{\giga\bit\per\second}).
...
Um cristal de \qty{12}{\mega\hertz}, ligado aos pinos 9 e 10, define o
...
16; 20}{\mega\hertz}. A escolha pelo cristal de \qty{12}{\mega\hertz} foi
arbitrária.
...
Uma porta \ac{USB} fornece \qty{5}{\volt} na via \VBUS, porém a especificação
...
inferiores a \qty{4.5}{\volt} \cite{ATmega8}.

Uma outra solução é manter o microcontrolador alimentado com \qty{5}{\volt} e
...
É composto de um diodo Zener de \qty{3.6}{\volt} ligando a linha de dados ao
\GND e um resistor de \qty{68}{\ohm} entre o microcontrolador e o diodo
Zener.

Quando o microcontrolador escreve o nível lógico 1, seu pino sobe para
\qty{5}{\volt}. Como essa tensão é maior que a tensão de ruptura do Zener,
este começa a conduzir, limitando a tensão da linha de dados. A diferença de
$\num{5} - \num{3.6} = \qty{1.4}{\volt}$ fica no resistor, resultando numa
corrente de $\qty{1.4}{\volt} / \qty{68}{\ohm} = \qty{20.6}{\milli\ampere}$,
que é abaixo do limite de \qty{40}{\milli\ampere} para cada pino do
...
deve ter \qty{1.5}{\kilo\ohm} \cite{usb20}. O projeto USBasp, no
entanto, usa um resistor de \qty{2.2}{\kilo\ohm} \cite{USBasp}, que, mesmo
...
\cite{HMC5883L}. Como a \ac{USB} fornece \qty{5}{\volt}, é necessário
adicionar um circuito para reduzir a tensão até um nível adequado.

A solução aqui empregada utiliza um diodo Zener de \qty{3.3}{\volt} e um
resistor de \qty{470}{\ohm} \cite{3vtipsandtricks}. É bastante similar àquela
...
drenando constantemente uma corrente de $ (\qty{5}{\volt} - \qty{3.3}{\volt})
/ \qty{470}{\ohm} = \qty{3.6}{\milli\ampere} $ e mantendo a tensão de
\qty{3.3}{\volt} em relação ao \GND entre esses dois componentes.

Considerando que o sensor drena apenas \qty{0.1}{\milli\ampere} quando em
...
\qty{3.6}{\volt} \cite{HMC5883L}, e tensão máxima tolerada no pino de
alimentação é de \qty{4.8}{\volt} \cite{HMC5883L}, que é abaixo da
tensão de \qty{5}{\volt} do microcontrolador. Isso significa que não é
prudente ligar os dois componentes diretamente.

Neste caso, faz-se necessário adicionar um circuito para converter o nível
lógico 1 entre \qty{5}{\volt} e \qty{3.3}{\volt}. Essa conversão precisa
...
\item[Nenhum dos dois dispositivos está puxando o barramento para zero] \hfill \\
	O lado de baixa tensão está em \qty{3.3}{\volt} devido ao resistor de
	...
	\qty{5}{\volt} devido ao resistor de \textit{pull-up}. Neste estado, os
...
\item[O dispositivo de menor tensão está puxando o barramento para zero] \hfill \\
	...
	\textit{source} é \qty{3.3}{\volt}, e o transistor entra em condução. O
...
chamada em intervalos menores que \qty{50}{\milli\second}, conforme descrito
...
sensor foi configurado para ±\qty{1.3}{\gauss}, o que corresponde a
...
}, ou \qty{0.92}{\milli\gauss} por bit. Este ganho foi escolhido por permitir
...
uma taxa máxima de \qty{75}{\hertz}. Apesar disso, é possível obter medições
a até \qty{160}{\hertz} se for configurado para o modo de medições
individuais e o pino DRDY for monitorado \cite{HMC5883L}. Este pino
normalmente fica no nível lógico 1, porém cai para 0 por
\qty{250}{\micro\second} assim que uma nova medida é colocada nos
...
\qty{500}{\micro\ampere} \cite{usbinanutshell}. O \textit{driver} não
...
endpoint} com intervalo \textit{polling} de \qty{10}{\milli\second} (o menor
...
intervalos menores que \qty{50}{\milli\second} para que o \textit{driver}
...
\qty{8.5}{\milli\second} \cite{ATmega8}. Portanto, a leitura é rápida
...
latitude. Há um espaçamento de \qty{1}{\degree} de longitude e
\qty{2}{\degree} de latitude entre os vetores.
...
\qty{1}{\degree} horizontalmente e \qty{2}{\degree} verticalmente.
...
aberturas pequenas, de até \qty{30}{\degree} ou \qty{45}{\degree}. As
distorções somente são acentuadas para aberturas maiores, a partir de
\qty{30}{\degree} ou \qty{60}{\degree}.
...
tende para o infinito quando a abertura tende para \qty{90}{\degree}.
