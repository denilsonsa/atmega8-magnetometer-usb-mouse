...
\newcommand*{\resultadoimagens}[1]{
	\includegraphics[width=0.19\textwidth]{../projection/images/P30T30p0t0_a#1.png}
	\includegraphics[width=0.19\textwidth]{../projection/images/P45T45p0t0_a#1.png}
	\includegraphics[width=0.19\textwidth]{../projection/images/P60T60p0t0_a#1.png}
	\includegraphics[width=0.19\textwidth]{../projection/images/P75T75p0t0_a#1.png}
	\includegraphics[width=0.19\textwidth]{../projection/images/P85T85p0t0_a#1.png}
}
...
compatível com um \textit{mouse} USB utilizando um microcontrolador AVR ATmega8 de 8
bits e um magnetômetro.
\end{resumo}

\begin{abstract}
%TODO: Corrigir o quê no abstract?
...
%TODO Neste capítulo são apresentadas blah blah
\newpage
...
\item Pode ser utilizado em situações onde um \textit{mouse} não é viável, situações
onde não há um apoio para um \textit{mouse} convencional.
...
movimentar um \textit{mouse}. O pequeno sensor pode ser preso à cabeça ou qualquer
...
para controlar o ponteiro do \textit{mouse} através de gestos. O produto é composto
...
e a transforma em movimentos do ponteiro do \textit{mouse}.
...
caso, em vez de controlar o ponteiro do \textit{mouse}, os movimentos da cabeça são
...
nos movimentos, o usuário precisa colocar na cabeça uma armação contendo
três pontos reflexivos ou três \acsp{LED} infravermelhos.
...
\textit{mouse} \acs{USB} e portanto funciona em qualquer sistema. Está à venda por
US\$~995,00.
...
TiltStick \cite{TiltStick} é um projeto que implementa um \textit{joystick} \acs{USB}
...
\textit{joysticks}. No entanto, essa classe foi projetada de forma genérica e inclui
...
intensidade do campo magnético usando o efeito magnetorresistivo em 3 eixos
perpendiculares (X, Y, Z) e converte essas medidas para um formato digital
...
toe.}''\\
{\small Ken Matusow}
% https://plus.google.com/107033731246200681024/posts/BryKKA16sH6
\end{flushright}{\small \par}
\vfill{}
...
ser alcançado, o \textit{firmware} foi dividido em diversos módulos.
...
e seus resultados é apresentado na seção \ref{sec:coordenadas}, que também
...
Development Software GmbH.}.\footnote{
	...
	mesmo tempo como \textit{mouse} e teclado. Não há nenhum par compartilhado de
...
Teclados e \textit{mouses} \ac{USB} podem ser implementados como dispositivos de
...
permitir o uso de teclados e \textit{mouses} mesmo nos estágios iniciais do
...
teclado ou como um \textit{mouse}, mas não como os dois. \cite[p.~9]{usbhid} Além
...
Cada uma das abordagens foi testada 5 vezes usando o conjunto de vetores
gerados pela ferramenta \texttt{generate\_sphere\_vectors.py} -- ferramenta
descrita na seção \ref{sub:ferramentas}, cujos vetores estão representados
...
o resultado de cada abordagem sendo testada com cada uma das 5 aberturas
citadas. Foram produzidas encadeando as três ferramentas auxiliares
descritas na seção \ref{sub:ferramentas} e mostram as deformações
...
O objetivo final deste projeto é controlar o ponteiro do \textit{mouse} na tela do
computador, e para isso foi preciso implementar um \textit{mouse} USB no
microcontrolador de maneira similar ao teclado USB descrito na seção
\ref{sec:keyboard}.

Todavia, o \textit{mouse} deste projeto tem uma diferença fundamental em relação aos
\textit{mouses} convencionais. A transformação de coordenadas, descrita na seção
...
uma posição específica da tela. Um \textit{mouse} convencional, no entanto, detecta
...
\textit{mouses} que implementem o protocolo \textit{boot}. \cite[p.~61]{usbhid} Assim
...
O \textit{report} usado por este \textit{mouse} possui 6 bytes. O primeiro deles é
o \textit{Report ID} e tem valor 2, identificando que este \textit{report}
corresponde ao \textit{report descriptor} do \textit{mouse}.\footnote{
...
\cite[p.~20]{usbhid} O último byte representa os botões do \textit{mouse}, sendo que
...
O resultado do primeiro teste após o a implementação do \textit{mouse} \ac{USB} e da
...
Durante os testes do \textit{mouse} \ac{USB}, foi descoberto um bug na implementação
...
%\textit{firmware} abaixo de 8192 bytes (tamanho da memória Flash do
%microcontrolador, conforme descrito na seção \ref{sec:atmega8}), e,
...
posição do ponteiro do \textit{mouse}. Mesmo depois de implementado um algoritmo de
...
%\lstinputlisting[language=Python, lineskip=-0.4\baselineskip]{../projection/convert_coordinates.py}


%\chapter{\texttt{main.c}}
%\label{ape:main}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/main.c}


\chapter{\texttt{keyemu.h} e \texttt{keyemu.c}}
\label{ape:keyemu}
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/keyemu.h}
\clearpage
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/keyemu.c}


\chapter{\texttt{mouseemu.h} e \texttt{mouseemu.c}}
\label{ape:mouseemu}
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/mouseemu.h}
\clearpage
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/mouseemu.c}


\chapter{\texttt{int\_eeprom.h} e \texttt{int\_eeprom.c}}
\label{ape:int_eeprom}
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/int_eeprom.h}
%\clearpage
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/int_eeprom.c}


\chapter{\texttt{sensor.h}}
%\chapter{\texttt{sensor.h} e \texttt{sensor.c}}
\label{ape:sensor}
\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/sensor.h}
%\clearpage
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/sensor.c}
...
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/buttons.h}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/buttons.c}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/common.h}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/main.c}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/menu.h}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/menu.c}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/hardwareconfig.h}
%\lstinputlisting[language=C, lineskip=-0.4\baselineskip]{../firmware/usbconfig.h}
