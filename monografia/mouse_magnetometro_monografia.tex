...
ferramenta. A figura \ref{fig:half_sphere} mostra um gráfico dos vetores 3D
impressos por esta ferramenta.
...
de um \textit{pipeline}.\footnote{
	Neste contexto, um \textit{pipeline} significa ligar a saída padrão de
	um programa à entrada padrão do programa seguinte.
}


\subsection{Usando geometria e ângulos}
...
$(x, y)$ contidas no plano da tela do computador. Todos esses vetores podem
ser vistos na figura \ref{fig:geometria_ABCD1}
...
figura \ref{fig:geometria_ABCD2}.
...
Para tal, primeiramente calcula-se o vetor unitário $N$, normal ao plano de
$A$ e $B$. São então calculados $P_N$, a componente de $P$ paralela ao vetor
$N$; e $P'$, a componente de $P$ contida no plano de $A$ e $B$.
...
por $A$ e $B$.  TODO FROM HERE


\subsection{Usando sistema de equações lineares}
\label{sub:coord_lineq}

De maneira similar à abordagem descrita na seção \ref{sub:coord_geometria},
esta abordagem também precisa calibrar os cantos da tela. Entretanto, apenas
os vetores $A$, $B$ e $D$ serão usados.
