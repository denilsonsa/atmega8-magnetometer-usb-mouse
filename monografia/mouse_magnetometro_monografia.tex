...
movimentos da câmera virtual dentro do ambiente simulado. Para obter 6 graus
de liberdade nos movimentos, o usuário precisa colocar na cabeça uma armação
contendo três pontos reflexivos ou três \acsp{LED} infravermelhos.
...
Além disso, ele define um \textit{report descriptor} que suporta até 6
...
O \textit{report} usado por este \textit{mouse} possui 6 bytes. O primeiro
deles é o \textit{Report ID} e tem valor 2, identificando que este
...
sendo que os 3 bits menos significativos correspondem aos 3 botões, e os 5
bits restantes são ignorados.
