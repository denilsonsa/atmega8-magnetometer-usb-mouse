...
O principal desafio teórico deste projeto é transformar os vetores
tridimensionais lidos do sensor em coordenadas bidimensionais da tela do
...
auxiliares em linguagem Python. A escolha da linguagem foi motivada pela
facilidade e rapidez de se escrever protótipos, assim como pela grande
biblioteca já disponível.

A primeira ferramenta foi chamada de \textt{generate\_sphere\_vectors.py} e
serve para simular leituras do sensor. Esta ferramenta imprime na saída
padrão vetores 3D que cobrem metade da superfície de uma esfera. Também
imprime 4 vetores correspondentes aos cantos da tela, vetores esses usados
nos cálculos explicados nas seções <XX> e <XX>.
% TODO \ref{sec:SECAO} e \ref{sec:SECAO}.

\begin{lstlisting}[numbers=none, float=h, label={lst:generate_sphere_vectors},
caption={\texttt{./generate\_sphere\_vectors.py}}
]
TODO...
\end{lstlisting}
